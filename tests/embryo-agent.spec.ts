...
describe('saved references', () => {
  const index = `# Second brain\n\nGenerated by the worker.\n\n## Design\n\n_UI patterns_\n\n- [Hover and Reveal Effects](design/hover-and-reveal-effects.md) — reveals on hover (3 sources)\n\n## Recent captures\n\n- 2026-09-17 · [Something](sources/2026-09/0072-x.md) → design/hover\n`
  const base = { seed: 'Fossils should be excavated, not browsed.', openTensions: [], dialogue: [], otherEmbryos: [] }

  it('adds the index while probing and while opening paths', () => {
    for (const state of ['GERMINATING', 'GROWING']) {
      const message = buildAgentUserMessage({ ...base, state, references: index })
      expect(message).toContain('Saved references')
      expect(message).toContain('- [Hover and Reveal Effects](design/hover-and-reveal-effects.md)')
      expect(message).not.toContain('Recent captures')
    }
  })

  it('never adds it while defining or closing', () => {
    for (const state of ['LATENT', 'MATURE']) {
      const message = buildAgentUserMessage({ ...base, state, references: index })
      expect(message).not.toContain('Saved references')
      expect(message).toBe(buildAgentUserMessage({ ...base, state }))
      expect(buildAgentSystemPrompt(state, { references: true })).toBe(buildAgentSystemPrompt(state))
    }
  })

  it('leaves the prompts untouched when there is no index', () => {
    expect(buildAgentUserMessage({ ...base, state: 'GROWING', references: '  ' }))
      .toBe(buildAgentUserMessage({ ...base, state: 'GROWING' }))
    expect(buildAgentSystemPrompt('GROWING', { references: false })).toBe(buildAgentSystemPrompt('GROWING'))
  })

  it('tells the agent a reference is a contrast, not a recommendation', () => {
    const prompt = buildAgentSystemPrompt('GROWING', { references: true })
    expect(prompt).toContain('A reference is a contrast, not a recommendation')
    expect(prompt).toContain('Never suggest adopting, using, trying, copying or reading a reference')
    expect(prompt).toContain('Preferred move: VARIETY')
  })
})
