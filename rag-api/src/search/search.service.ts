...
export class SearchService {
  ...
  async rag(query: string, limit: number = 5): Promise<{
    query: string;
   ...
   */
  async inspect(query: string, limit: number = 5): Promise<{
    query: string;
...
${context || '(no matching passages found)'}`;
  ...
  async generateChatReply(
    ...
    const ms = Math.max(
      30_000,
      timeoutMs ?? (Number(this.config.get('OLLAMA_CHAT_TIMEOUT_MS')) || 180_000),
    );
  ...
  async logRagQuery(input: {
    ...
    try {
      ...
      this.logger.warn(`Failed to persist Query telemetry: ${err?.message || err}`);
    }
  }
  ...
  async converse(dto: ConverseDto): Promise<{
    ...
    const { context, sources, results } = await this.rag(queryText, limit);
