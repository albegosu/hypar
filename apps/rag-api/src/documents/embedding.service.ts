...
import {
  createOllama,
  normalizeOllamaNativeHost,
} from "../ollama/create-ollama";
...
export class EmbeddingService {
  ...
  constructor(private config: ConfigService) {
    ...
    this.dimensions =
      Number.isFinite(rawDims) && rawDims > 0 ? rawDims : DEFAULT_DIMENSIONS;
    ...
    this.logger.log(
      `Using embedding provider: ${this.useGoogleAI ? "Google Gemini" : this.useOpenAI ? "OpenAI" : "Ollama"}`,
    );
  }
  ...
  private async generateOllama(text: string): Promise<number[]> {
    try {
      ...
      if (
        message.includes("/api/embeddings") &&
