...
export class DocumentsService {
  ...
  async createFromText(dto: CreateDocumentDto) {
    ...
    const userId = dto.userId ? this.sanitizePgText(dto.userId) : null;
  ...
  /**
   * Persist a chat "fact" into the user's local memory (stored as a normal
   * Document+Chunks+Embeddings so it can be retrieved via pgvector).
   */
  async createChatMemory(userId: string | undefined, content: string) {
    const text = this.sanitizePgText(content).trim();
    if (!text) return null;
  ...
  async deleteChatMemories(userId: string | undefined, term?: string) {
    ...
    // Cascades: Document -> Chunk via ON DELETE CASCADE
    return this.prisma.$executeRaw(Prisma.sql`DELETE FROM "Document" ${where}`);
  }

  async chatMemoryExists(
    userId: string | undefined,
    contentSnippet: string,
  ): Promise<boolean> {
    const snippet = this.sanitizePgText(contentSnippet).trim();
  ...
  private async processDocument(documentId: string, content: string) {
    ...
    for (let start = 0; start < chunks.length; start += concurrency) {
      ...
      const batch = await Promise.all(
        slice.map((c) => this.embedding.generate(c.content)),
      );
  ...
  private async extractTextFromUpload(
    ...
    if (this.isPdfUpload(file)) {
      try {
        ...
        this.logger.warn(
          `pdf-parse failed, falling back to UTF-8 decode: ${err?.message || err}`,
        );
      }
