---
layout: home

hero:
  name: 'hypar<span class="home-hero-name__caret" aria-hidden="true"></span>'
  text: This is a Resizes lab_project
  tagline: Open-source reference implementation of production RAG in TypeScript.
  actions:
    - theme: brand
      text: Try demo
      link: https://from-zero-rag-production.up.railway.app/
    - theme: alt
      text: Read tutorial
      link: /guide/getting-started
    - theme: alt
      text: View on GitHub
      link: https://github.com/albegosu/from-zero-rag

features:
  - title: Library
    icon:
      svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l8 4.5v9L12 21l-8-4.5v-9z"/><path d="M12 12l8-4.5"/><path d="M12 12v9"/><path d="M12 12L4 7.5"/></svg>'
    details: Production patterns — hybrid search, MMR, durable ingest, Vitest coverage. Golden-set eval harness is on the roadmap.
  - title: Tutorial
    icon:
      svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M19 4h-9a3 3 0 0 0-3 3v13a2 2 0 0 1 2-2h10z"/><path d="M19 4v16"/><path d="M9 8h6"/><path d="M9 12h4"/></svg>'
    details: This VitePress site plus the app README (EN). UI i18n includes ES. The old Monaco `/learn` quest is not in the current app — see Learning quest for history and plans.
  - title: Portfolio
    icon:
      svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="M7 14l4-4 3 3 5-6"/></svg>'
    details: Roadmaps in-repo; public benchmarks per release are a v1.0 goal (hit-rate, MRR, faithfulness, cost).
---
